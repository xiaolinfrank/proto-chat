...
export class AiInfraRepos {
  ...
  getAiProviderList = async () => {
    ...
    const builtinProviders = DEFAULT_MODEL_PROVIDER_LIST.map((item) => ({
      ...
      enabled:
        ...
        // Fall back to the enabled attribute in DEFAULT_MODEL_PROVIDER_LIST (for special providers like protochat)
        item.enabled,
      id: item.id,
      name: item.name,
      source: 'builtin',
  ...
  getAiProviderRuntimeState = async (
    ...
    const enabledImageAiProviders = enabledAiProviders.filter((provider) => {
      // Only allow protochat provider to supply image models, to avoid displaying built-in image models from other providers

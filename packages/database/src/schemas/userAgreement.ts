...
export const userAgreements = pgTable('user_agreements', {
  ...
  // Merchant agreement number (generated by us, uniquely identifies a user agreement)
  externalAgreementNo: varchar('external_agreement_no', { length: 64 }).unique().notNull(),
  // Sign scene, e.g. INDUSTRY|DIGITAL_MEDIA
  signScene: varchar('sign_scene', { length: 64 }).notNull(),
  // Sign channel: alipay | wechat
  ...
  // ============ Agreement Status ============
  // pending: awaiting agreement, signed: agreement signed, unsigned: agreement cancelled
  status: varchar('status', { length: 20 }).default('pending').notNull(),
  // Agreement sign time
  signTime: timestamp('sign_time'),
  // Agreement cancellation time
  unsignTime: timestamp('unsign_time'),
  // Cancellation reason: user_unsign (user actively cancelled), merchant_unsign (merchant cancelled), expired (expired)

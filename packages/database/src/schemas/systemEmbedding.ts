...
export const systemEmbeddingConfig = pgTable('system_embedding_config', {
  ...
  // Pricing (credits/million tokens)
 ...
 * Stores embedding model information synced from providers, for selection only
 */
export const systemEmbeddingModels = pgTable(
  'system_embedding_models',
  {
    ...
    // Pricing information (from sync API)
    inputPrice: decimal('input_price', { precision: 15, scale: 4 }),
    currency: varchar('currency', { length: 10 }).default('USD'),
 ...
 * Records detailed logs of users using embedding, for cost analysis and auditing
 */
export const embeddingUsageLogs = pgTable(
  'embedding_usage_logs',
  {
    ...
    // Cost records (using scale: 8 to support extremely small cost values, e.g. $0.00000001)
    ...
    fileId: text('file_id'), // If it's a file embedding (corresponds to files.id - text type)

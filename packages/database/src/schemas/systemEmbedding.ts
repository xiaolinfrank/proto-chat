 ...
 * Stores embedding model information synchronized from providers, for selection purposes only
 */
export const systemEmbeddingModels = pgTable(
  'system_embedding_models',
  {
    ...
    // Pricing information (retrieved from sync API)
    inputPrice: decimal('input_price', { precision: 15, scale: 4 }),
    currency: varchar('currency', { length: 10 }).default('USD'),
...
export const embeddingUsageLogs = pgTable(
  'embedding_usage_logs',
  {
    ...
    fileId: text('file_id'), // if it is a file embedding (corresponds to files.id - text type)

 ...
 * Stores the globally configured embedding model
 */
 ...
 * Stores embedding model information synced from providers, used only for selection
 */
 ...
 * Records detailed logs of users' embedding usage for cost analysis and auditing
 */
export const embeddingUsageLogs = pgTable(
  'embedding_usage_logs',
  {
    ...
    // Cost records (scale: 8 to support very small cost values such as $0.00000001)
    ...
    fileId: text('file_id'), // references files.id (text type) when embedding a file
    chunkCount: integer('chunk_count'), // number of chunks in a batch embedding

 ...
 * Stores API configurations for underlying providers such as OpenRouter, DeepSeek, etc.
 */
export const protochatProviders = pgTable(
  'protochat_providers',
  {
    ...
    // 'aggregator' = aggregator platform (e.g. OpenRouter), 'direct' = direct provider (e.g. DeepSeek)
    type: varchar('type', { length: 50 }).notNull(),
    enabled: boolean('enabled').default(true).notNull(),
    // Priority, used for sorting when selecting a provider
    priority: integer('priority').default(0),

    // Provider alias used for generating model IDs, hiding the real provider name
    // Format such as a1, b2, c3, used in model ID: protochat::{alias}::{modelId}
    ...
    // 'api' = fetch from provider API, 'model_bank' = fetch from Model-Bank, 'manual' = manually configured
 ...
 * Stores models available for ProtoChat providers and their mapping relationships to underlying providers
 */
export const protochatModels = pgTable(
  'protochat_models',
  {
    ...
    // Whether enabled in the admin panel (controls user visibility)
 ...
 * Records detailed logs of user usage of ProtoChat models, used for analysis and auditing
 */

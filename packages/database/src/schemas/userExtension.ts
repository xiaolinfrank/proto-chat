...
export const userExtensions = pgTable(
    'user_extensions',
    {
        ...
        // Next billing cycle preset plan ID (for mid-cycle downgrade or cancellation)
        nextPlanId: text('next_plan_id'),

        // ============ Contract Info ============
        // Currently active contract agreement ID
        currentAgreementId: text('current_agreement_id'),
        // Auto-renewal status: true=enabled, false=disabled
        autoRenew: boolean('auto_renew').default(false),

        // ============ Downgrade Info ============
        // Downgrade reason: deduct_failed(payment failed), user_unsign(user cancelled), expired(subscription not renewed)

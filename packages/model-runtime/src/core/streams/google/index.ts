...
const transformGoogleGenerativeAIStream = (
  ...
  if (candidate) {
        ...
        { data: text, id: context.id, type: 'text' },
        {
          data: {
            citations: groundingChunks?.map((chunk) => ({
              // The URI returned by Google has been processed by Google itself, so the real favicon cannot be displayed
              // Use title as a replacement
    ...
    if (candidate.finishReason) {
      ...
      // When there is a finishReason but no text content, send an empty text chunk to stop the loading animation
      return [

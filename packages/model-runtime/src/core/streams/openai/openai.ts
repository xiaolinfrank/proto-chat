...
const transformOpenAIStream = (
  ...
  // MiniMax returns business errors (e.g. insufficient balance) in base_resp, but does not go through FIRST_CHUNK_ERROR_KEY
  // Typical return: { id: '...', choices: null, base_resp: { status_code: 1008, status_msg: 'insufficient balance' }, usage: {...} }
  if ((chunk as any).base_resp && typeof (chunk as any).base_resp.status_code === 'number') {
    ...
    if (baseResp.status_code !== 0) {
      // Map MiniMax error code to the corresponding error type
      ...
      switch (baseResp.status_code) {
        ...
        // 1002 - Request rate limit exceeded / 1041 - Connection limit / 2045 - Request rate growth exceeded
        case 1002:
        case 1041:
        case 2045: {
  ...
  try {
    ...
    if (item.finish_reason) {
      ...
      if (typeof item.delta?.content === 'string' && !!item.delta.content) {
        // MiniMax built-in search returns citation sources in content of the first tools stream, which should be ignored
      ...
      // MiniMax built-in search returns 4 objects in the messages array of the last stream, the last of which is annotations
    ...
    if (item.delta) {
      ...
      if (typeof content === 'string') {
        // If content is an empty string but the chunk has usage, return usage first (e.g. Gemini image-preview eventually returns usage in a separate chunk)
        if (content === '' && chunk.usage) {
          const usage = chunk.usage;
          return { data: convertOpenAIUsage(usage, payload), id: chunk.id, type: 'usage' };
        }

        // Handle the special case containing </think> tags: content needs to be split
        if (content.includes('</think>')) {
          const parts = content.split('</think>');
          const beforeThink = parts[0].replaceAll('<think>', ''); // Remove possible <think> tags
          const afterThink = parts.slice(1).join('</think>'); // Handle the case where there may be multiple </think>
        ...
        // Remove <think> tags (no need to split, because content after <think> tag is all reasoning)
        const thinkingContent = content.replaceAll(/<\/?think>/g, '');

        // Check if there is a <think> tag, update thinkingInContent state
        if (content.includes('<think>')) {
          streamContext.thinkingInContent = true;
        }

        // Check if there is citations content, update returnedCitation state
            ...
            const baseChunks: StreamProtocolChunk[] = [
              {
                data: {
                  citations: (citations as any[])
                    ...
                    .filter((c) => c.title && c.url), // Zhipu built-in search tool sometimes returns empty links which can cause crashes
                },
                id: chunk.id,
                type: 'grounding',
              },
        ...
        // In non-thinking mode, additionally parse base64 images in markdown and output in order: text -> base64_image
        ...
        // Determine the return type based on the current thinking mode
        return {
          data: thinkingContent,
          id: chunk.id,
          type: streamContext?.thinkingInContent ? 'reasoning' : 'text',
    ...
    // In litellm's return results, there are cases where delta is empty but usage is present
    if (chunk.usage) {
      const usage = chunk.usage;
      return { data: convertOpenAIUsage(usage, payload), id: chunk.id, type: 'usage' };
    }

    // In all other cases, return delta and index
    return {

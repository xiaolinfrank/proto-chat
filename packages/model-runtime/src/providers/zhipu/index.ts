...
export const params = {
  ...
  chatCompletion: {
      ...
      const zhipuTools = enabledSearch
        ? [
            ...(tools || []),
            {
              ...
              web_search: {
                enable: true,
                result_sequence: 'before', // Change search result return order to 'before' to minimize OpenAIStream changes
    ...
    handleStream: (stream, { callbacks, inputStartAt }) => {
      ...
      // GLM-4.5 series models return index -1 in tool_calls, which must be corrected before entering OpenAIStream
      ...
      const preprocessedStream = readableStream.pipeThrough(
        new TransformStream({
          transform(chunk, controller) {
            // Process the original OpenAI ChatCompletionChunk format
            if (chunk.choices && chunk.choices[0]) {
              const choice = chunk.choices[0];
              if (choice.delta?.tool_calls && Array.isArray(choice.delta.tool_calls)) {
                // Fix negative index, convert -1 to a positive index based on array position

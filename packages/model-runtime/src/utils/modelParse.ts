...
export interface ModelProcessorConfig {
  excludeKeywords?: readonly string[]; // Do not add labels to matching models
...
export const IMAGE_MODEL_KEYWORDS = [
  ...
  'DESCRIBE',
  'UPSCALE',
  '!gemini', // Exclude gemini models, even if they contain -image they are still chat models
  '-image',
  '^V3',
  '^V_2',
  '^V_1',
...
/**
 * Check if a keyword list matches a model ID (supports multiple match modes)
 * @param modelId Model ID (lowercase)
 * @param keywords Keyword list, supporting the following prefixes:
 *   - ^ prefix: match only at the start of the model ID
 *   - ! prefix: exclusion match, highest priority
 *   - no prefix: substring match (default behavior)
...
const isKeywordListMatch = (modelId: string, keywords: readonly string[]): boolean => {
  // First check exclusion rules (starting with !)
  ...
  // If any exclusion rule matches, return false directly
  ...
  return includeKeywords.some((keyword) => {
    if (keyword.startsWith('^')) {
      // ^ prefix means match only at the start
...
/**
 * Find the local model configuration matching the given provider type
...
const findKnownModelByProvider = async (
  ...
  try {
    ...
    // If the provider configuration file does not exist, skip
    ...
    // If the import succeeded and has data, perform the search
    ...
    // If the import fails (file not found or other error), return null
    return null;
  }
};
...
const formatTimestampToDate = (timestamp: number): string | undefined => {
  ...
  // - Otherwise treat as seconds, multiply by 1000 to convert to milliseconds
  const msTimestamp = timestamp > 1e12 ? timestamp : timestamp * 1000;
  const date = new Date(msTimestamp);

  // Validate the parsed result and year range (only accept 4-digit years to avoid exceeding the YYYY-MM-DD limit of varchar(10))
...
const processReleasedAt = (model: any, knownModel?: any): string | undefined => {
  ...
  if (model.created !== undefined && model.created !== null) {
    // Check if it is in timestamp format
    if (typeof model.created === 'number' && model.created > 1_630_000_000) {
      // AiHubMix has an incorrect timestamp of 1626777600
    ...
    if (typeof model.created === 'string') {
      // Anthropic: if in '2025-02-19T00:00:00Z' format, extract the date part only
...
const getProviderLocalConfig = async (provider?: ModelProviderKey): Promise<any[] | null> => {
  ...
  if (provider) {
    try {
      ...
      // If the config file does not exist or the import fails, keep as null
...
/**
 * Get the local enable configuration for a model
...
const getModelLocalEnableConfig = (
  ...
  // If a providerid is provided and local config exists, try to get the model's enabled state from it
...
/**
 * Process the model list for a single provider
 * @param modelList Model list
 * @param config Provider configuration
 * @param provider Provider type (optional, used to prioritize matching the corresponding local config; when provider is given, the enabled state from local config will be applied)
 * @returns Processed list of model cards
 */
export const processModelList = async (
  ...
  // If provider is given, try to get the local configuration for that provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // If provider is given, prioritize using the provider-specific configuration
      if (provider) {
        knownModel = await findKnownModelByProvider(model.id, provider);
      }

      // If not found, fall back to the global configuration
      if (!knownModel) {
      ...
      // If provider is given and local config exists, try to get the model's enabled state from it
      const providerLocalModelConfig = getModelLocalEnableConfig(
        providerLocalConfig as any[],
        model,
      );

      // If the model is found in the local configuration, use its enabled state
      if (
        processedModel &&
...
/**
 * Process the model list for mixed providers
 * @param modelList Model list
 * @param providerid Optional provider ID, used to retrieve its local configuration file
 * @returns Processed list of model cards
 */
export const processMultiProviderModelList = async (
  ...
  // If providerid is given, try to get the local configuration for that provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // Prioritize using the provider-specific configuration
      ...
      // If providerid is given and local config exists, try to get the model's enabled state from it
      const providerLocalModelConfig = getModelLocalEnableConfig(
        providerLocalConfig as any[],
        model,
      );

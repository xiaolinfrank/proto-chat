 ...
 * @returns Whether it matches (exclusion logic has priority)
 */
const isKeywordListMatch = (modelId: string, keywords: readonly string[]): boolean => {
  // First check exclusion rules (starts with exclamation mark)
  ...
  return includeKeywords.some((keyword) => {
    if (keyword.startsWith('^')) {
      // ^ prefix means only match at the beginning
...
const formatTimestampToDate = (timestamp: number): string | undefined => {
  ...
  // - If millisecond-level (>= 1e12), use directly as milliseconds
  ...
  // Validate parsing result and year range (only accept 4-digit years, avoid exceeding varchar(10) YYYY-MM-DD)
...
const processDisplayName = (displayName: string): string => {
  // If contains "Gemini 2.5 Flash Image Preview", replace that part with "Nano Banana"
...
const getModelLocalEnableConfig = (
  ...
  // If provider ID is provided and has local configuration, try to get the model's enabled status
...
/**
 * General logic for processing model cards
 */
 ...
 * @param provider Provider type (optional, used to prioritize matching the corresponding local configuration; when provider is provided, will try to override enabled from local configuration)
...
export const processModelList = async (
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // If provider is provided and has local configuration, try to get the model's enabled status
 ...
 * @param modelList Model list
 * @param providerid Optional provider ID for getting its local configuration file
 * @returns Processed model card list
 */
export const processMultiProviderModelList = async (
  ...
  // If provider ID is provided, try to get the provider's local configuration

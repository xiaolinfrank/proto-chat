...
/**
 * Checks if a keyword list matches the model ID (supports multiple matching modes)
 ...
 *   - no prefix: inclusion match (default behavior)
...
const isKeywordListMatch = (modelId: string, keywords: readonly string[]): boolean => {
  ...
  // If it matches any exclusion rule, return false directly
  ...
  return includeKeywords.some((keyword) => {
    ...
    // Default behavior: inclusion match
    return modelId.includes(keyword);
  });
};

/**
 * Find local model configuration based on provider type
...
const findKnownModelByProvider = async (
  ...
  try {
    ...
    // If the import succeeds and has data, perform a search
 ...
 * @param modelId Model ID
 * @returns The detected provider configuration key, defaults to 'openai'
 */
...
const formatTimestampToDate = (timestamp: number): string | undefined => {
  ...
  // - If millisecond-level (>= 1e12), treat it directly as milliseconds;
  // - Otherwise treat as seconds, need to multiply by 1000 to convert to milliseconds
  const msTimestamp = timestamp > 1e12 ? timestamp : timestamp * 1000;
  const date = new Date(msTimestamp);

  // Validate parse result and year range (only accept 4-digit years to avoid exceeding varchar(10) for YYYY-MM-DD)
...
const processReleasedAt = (model: any, knownModel?: any): string | undefined => {
  // Preferentially check model.created
  if (model.created !== undefined && model.created !== null) {
    ...
    if (typeof model.created === 'string') {
      // Anthropic: if it is '2025-02-19T00:00:00Z', only take the date part
 ...
 * @returns The local configuration for the model provider
 */
const getProviderLocalConfig = async (provider?: ModelProviderKey): Promise<any[] | null> => {
  ...
  if (provider) {
    try {
      ...
      // If the configuration file does not exist or import fails, keep as null
...
/**
 * Get the local model configuration
 * @param providerLocalConfig Local configuration for the model provider
 * @param model Model object
 * @returns The local model configuration
 */
const getModelLocalEnableConfig = (
  providerLocalConfig: any[],
  model: { id: string },
): any | null => {
  // If providerid is provided and local config exists, try to get the model's enabled state from it
 ...
 * @param provider Provider type (optional, used to preferentially match the corresponding local config; when provider is provided, the enabled state from local config will be applied)
...
export const processModelList = async (
  ...
  // If provider is provided, try to get the local configuration for this provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // If provider is provided, preferentially use provider-specific configuration
      if (provider) {
      ...
      // If provider is provided and local config exists, try to get the model's enabled state from it
      const providerLocalModelConfig = getModelLocalEnableConfig(
        providerLocalConfig as any[],
        model,
      );

      // If the model is found in local config, use its enabled state
      if (
        processedModel &&
 ...
 * @param modelList Model list
 * @param providerid Optional provider ID, used to obtain its local configuration file
 * @returns Processed model card list
 */
export const processMultiProviderModelList = async (
  ...
  // If providerid is provided, try to get the local configuration for this provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // Preferentially use provider-specific configuration

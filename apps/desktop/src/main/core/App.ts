...
export class App {
  ...
  constructor() {
    ...
    // Uniformly handle the before-quit event
  ...
  async handleProtocolRequest(urlType: string, action: string, data: any): Promise<boolean> {
    ...
    try {
      ...
      return result !== false; // Assume the controller returns false to indicate a failed handle
    } catch (error) {
  ...
  // Added before-quit handler function

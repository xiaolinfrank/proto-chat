...
/**
 * Validate the MCP Schema object structure
 */
 ...
 * Responsible for handling the MCP plugin installation flow
 */
export default class McpInstallController extends ControllerModule {
  ...
  public async handleInstallRequest(parsedData: McpInstallParams): Promise<boolean> {
    try {
      ...
      // If a schema parameter is provided, parse and validate it
      if (schemaParam) {
        try {
        ...
        // Validate that identifier matches the id parameter
      ...
      // Broadcast the installation request to the frontend
      ...
      // Broadcast to the frontend via the app instance

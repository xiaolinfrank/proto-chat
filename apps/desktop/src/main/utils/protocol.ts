...
export const getProtocolScheme = (): string => {
  // The version can be determined in multiple ways within the Electron environment
  const bundleId = app.name;
  const appPath = app.getPath('exe');

  // Determine via bundle identifier
 ...
 * @returns Whether the schema is a valid MCP Schema
 */
...
/**
 * Parse a lobehub:// protocol URL (supports multiple version schemes)
 *
 ...
 * @returns Parsed result containing the basic structure and all query parameters
 */
...
/**
 * Generate a protocol URL conforming to RFC 0001
 *
...
export function generateRFCProtocolUrl(params: {
  ...
  // Validate that schema.identifier matches id
  ...
  // Encode schema — pass JSON string directly and let URLSearchParams auto-encode

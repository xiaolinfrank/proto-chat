...
export const getProtocolScheme = (): string => {
  // In Electron, the version can be determined in multiple ways
 ...
 * @returns Whether this is a valid MCP Schema
 */
 ...
 * @returns Parsed result containing the base structure and all query parameters
 */
export const parseProtocolUrl = (url: string): ProtocolUrlParsed | null => {
  try {
    ...
    // After parsing a custom protocol URL:
...
export function generateRFCProtocolUrl(params: {
  ...
  // Encode schema - pass JSON string directly and let URLSearchParams auto-encode it

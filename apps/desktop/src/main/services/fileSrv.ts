...
export default class FileService extends ServiceModule {
   ...
   * @deprecated Only for backward compatibility with legacy file access, new files should be stored in custom paths under FILE_STORAGE_DIR
   */
  get UPLOADS_DIR() {
  ...
  async uploadFile({
    ...
    try {
      // Get current timestamp to avoid repeated Date.now() calls
      const now = Date.now();
      const date = (now / 1000 / 60 / 60).toFixed(0);

      // Use the passed-in filePath as the file storage path
  ...
  /**
   * Determine if path is in legacy format (timestamp directory)
   *
   * Legacy path format: {timestamp}/{hash}.{ext} (e.g., 1234567890/abc123.png)
   * New path format: arbitrary custom path (e.g., user_uploads/images/photo.png, ai_generations/image.jpg)
   *
  ...
  private isLegacyPath(path: string): boolean {
    ...
    // If the first part is pure digits (timestamp), consider it legacy format
  ...
  async getFile(path: string): Promise<{ content: ArrayBuffer; mimeType: string }> {
    ...
    try {
      ...
      // Possible received formats: desktop:/12345/file.png or desktop://12345/file.png
      ...
      // Smart routing: determine which directory to read file from based on path format
      let filePath: string;
      ...
      try {
        ...
        // If metadata file doesn't exist, try guessing MIME type from file extension
      ...
      // If it's a file not found error, throw custom FileNotFoundError
  ...
  async deleteFile(path: string): Promise<{ success: boolean }> {
    ...
    try {
      ...
      // Smart routing: determine which directory to delete file from based on path format
      let filePath: string;
      let isLegacyAttempt = false;
      ...
      // Delete file and metadata, if first attempt fails and is legacy path, try new path
  ...
  async getFilePath(path: string): Promise<string> {
    ...
    // Smart routing: determine which directory to get file path from based on path format
    let fullPath: string;
    if (this.isLegacyPath(relativePath)) {
      ...
      try {
        ...
        try {
          ...
          // Neither path exists, return original legacy path (maintain original behavior)
          logger.debug(
  ...
  async getFileHTTPURL(path: string): Promise<string> {
    ...
    // Use StaticFileServerManager to get file server domain, then build complete URL

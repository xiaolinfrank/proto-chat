...
export const getJWKS = (): object => {
  try {
    ...
    // Try to parse JWKS JSON string
    const jwks = JSON.parse(jwksString);

    // Check if JWKS format is correct
    if (!jwks.keys || !Array.isArray(jwks.keys) || jwks.keys.length === 0) {
      throw new Error('JWKS 格式无效: 缺少或为空的 keys 数组');
    }

    // Check if there is a key with RS256 algorithm
...
const getVerificationKey = async () => {
  try {
    ...
    // The key fields for an RSA public key are kty, n, e. Others such as kid, alg, use are also public.
    const publicKeyJwk = {
    ...
    // Remove any potentially undefined fields to keep the object clean
...
export const validateOIDCJWT = async (token: string) => {
  try {
    ...
    const { payload } = await jwtVerify(token, publicKey, {
      algorithms: ['RS256'],
      // Additional validation options such as issuer and audience can be added here
    });

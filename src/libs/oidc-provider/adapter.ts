...
// Create adapter debug namespace
...
class OIDCAdapter {
  ...
  /**
   * Get the corresponding database table by model name
   */
  private getTable() {
  ...
  async upsert(id: string, payload: any, expiresIn: number): Promise<void> {
    ...
    if (this.name === 'Client') {
      // Special handling for Client model, directly use the provided data
    ...
    // For other models, save the full data and metadata
    ...
    if (payload.accountId) {
      ...
      try {
        ...
        try {
          ...
          // If getting userId fails, continue processing without throwing
        }
      } catch (importError) {
        log('[%s] Error importing auth module: %O', this.name, importError);
        // If importing the module fails, continue processing without throwing
      }
    }
  ...
  async find(id: string): Promise<any> {
    ...
    try {
      ...
      // If the record has expired, return undefined
      ...
      // If the record has been consumed, check whether it is within the grace period
      if (model.consumedAt) {
  ...
  /**
   * Find a model instance by userCode (for device flow only)
   */
  async findByUserCode(userCode: string): Promise<any> {
    ...
    try {
      ...
      // If the record has expired or been consumed, return undefined
      if (model.expiresAt && new Date() > new Date(model.expiresAt)) {
  ...
  /**
   * Find an interaction instance by uid
   */
    ...
    if (this.name === 'Session') {
      try {
        ...
        if (model.expiresAt && model.expiresAt < new Date()) {
          ...
          await this.destroy(model.id); // Still delete using the primary key id
          return undefined;
        }
  ...
  /**
   * Mark a model instance as consumed
   */
  ...
  async revokeByGrantId(grantId: string): Promise<void> {
    ...
    // Grants themselves do not need to be revoked by grantId
    if (this.name === 'Grant') {
      log('[Grant] revokeByGrantId skipped for Grant model, as it is the grant itself');
      return;
    }

    // Validate the model name early, even if the table is not used directly afterward
    this.getTable();

    try {
      log('[%s] Starting transaction for revokeByGrantId operations', this.name);

      // Use a transaction to delete all records containing the grantId, ensuring atomicity
      await this.db.transaction(async (tx) => {
        // All tables that may contain a grantId
        const tables = [
          oidcAccessTokens,
          oidcAuthorizationCodes,
          oidcRefreshTokens,

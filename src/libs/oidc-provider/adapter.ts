...
class OIDCAdapter {
  ...
  async upsert(id: string, payload: any, expiresIn: number): Promise<void> {
    ...
    if (this.name === 'Client') {
      // Special handling for Client model, use the passed-in data directly
    ...
    if (payload.accountId) {
      ...
      try {
        ...
        try {
          ...
          // If getting userId fails, continue processing without throwing an error
        }
      } catch (importError) {
        log('[%s] Error importing auth module: %O', this.name, importError);
        // If importing module fails, continue processing without throwing an error
      }
    }
  ...
  async find(id: string): Promise<any> {
    ...
    try {
      ...
      // If the record has been consumed, check if it's within the grace period
      if (model.consumedAt) {
  ...
  async findByUid(uid: string): Promise<any> {
    ...
    if (this.name === 'Session') {
      try {
        ...
        if (model.expiresAt && model.expiresAt < new Date()) {
          ...
          await this.destroy(model.id); // Still delete using primary key id
          return undefined;
        }
    ...
    // Reuse the find method implementation
  ...
  async revokeByGrantId(grantId: string): Promise<void> {
    ...
    // Grants themselves don't need to be revoked via grantId
    if (this.name === 'Grant') {
      log('[Grant] revokeByGrantId skipped for Grant model, as it is the grant itself');
      return;
    }

    // Pre-validate model name even if table is not used directly afterward
    this.getTable();

    try {
      log('[%s] Starting transaction for revokeByGrantId operations', this.name);

      // Delete all records containing grantId using a transaction to ensure atomicity

...
class OIDCAdapter {
  ...
  async upsert(id: string, payload: any, expiresIn: number): Promise<void> {
    ...
    if (this.name === 'Client') {
      // Special handling for client model, use the passed data directly
    ...
    // For other models, save full data and metadata
    ...
    if (payload.accountId) {
      ...
      try {
        ...
        // If importing the module fails, continue processing without throwing an error
      }
    }
  ...
  async revokeByGrantId(grantId: string): Promise<void> {
    ...
    // Grants themselves do not need to be revoked via grantId
    if (this.name === 'Grant') {
      log('[Grant] revokeByGrantId skipped for Grant model, as it is the grant itself');
      return;
    }

    // Validate model name early, even if table is not used directly afterward
    this.getTable();

    try {
      log('[%s] Starting transaction for revokeByGrantId operations', this.name);

      // Use a transaction to delete all records containing grantId, ensuring atomicity

...
export const createOIDCProvider = async (db: LobeChatDatabase): Promise<Provider> => {
  ...
  const configuration: Configuration = {
    ...
    // New: Ensure ID Token includes claims for all scopes, not just the openid scope
    ...
    interactions: {
      ...
      url(ctx, interaction) {
        ...
        // Read the ui_locales parameter from the OIDC request (space-separated language priority list)
        ...
        let query = '';
        if (uiLocalesRaw) {
          // Take the first preferred language, normalize to a tag supported by the site

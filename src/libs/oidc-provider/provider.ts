...
/**
 * Get the Cookie secret using KEY_VAULTS_SECRET
 */
const getCookieKeys = () => {
  ...
  const configuration: Configuration = {
    ...
    clientBasedCORS(ctx, origin, client) {
      ...
      if (!client || !client.redirectUris) {
        ...
        return false; // Deny if no client or redirect URIs are available
      }
      ...
      const allowed = client.redirectUris.some((uri) => {
        try {
          ...
          // Skip if redirect_uri is not a valid URL (e.g., custom scheme)
          return false;
        }
      });
    ...
    features: {
      ...
      resourceIndicators: {
        ...
        getResourceServerInfo: (ctx, resourceIndicator) => {
          logProvider('getResourceServerInfo called with indicator: %s', resourceIndicator); // <-- Added log
          if (resourceIndicator === API_AUDIENCE) {
            logProvider('Indicator matches API_AUDIENCE, returning JWT config.'); // <-- Added log
            return {
    ...
    async findAccount(ctx: KoaContextWithOIDC, id: string) {
      ...
      // Check for a pre-stored external account ID
      ...
      // If no available ID, return undefined
      if (!accountIdToFind) {
    ...
    interactions: {
      ...
      url(ctx, interaction) {
        ...
        // Read the ui_locales parameter from the OIDC request (space-separated locale priority)
        ...
        let query = '';
        if (uiLocalesRaw) {
          // Take the first preferred locale and normalize it to a site-supported tag

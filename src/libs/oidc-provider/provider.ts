...
export const createOIDCProvider = async (db: LobeChatDatabase): Promise<Provider> => {
  ...
  const configuration: Configuration = {
    ...
    clientBasedCORS(ctx, origin, client) {
      ...
      // A common strategy is to allow the origins of all registered redirect_uris
      if (!client || !client.redirectUris) {
        logProvider('clientBasedCORS: No client or redirectUris found, denying origin: %s', origin);
        return false; // Deny if no client or redirect URIs are found
      }
      ...
      const allowed = client.redirectUris.some((uri) => {
        try {
          ...
          // If redirect_uri is not a valid URL (e.g., custom protocol), skip it
          return false;
        }
      });
    ...
    // Added: ensure ID Token includes claims for all scopes, not just the openid scope
    ...
    features: {
      ...
      resourceIndicators: {
        ...
        getResourceServerInfo: (ctx, resourceIndicator) => {
          logProvider('getResourceServerInfo called with indicator: %s', resourceIndicator); // <-- added this log line
          if (resourceIndicator === API_AUDIENCE) {
            logProvider('Indicator matches API_AUDIENCE, returning JWT config.'); // <-- added this log line
            return {
    ...
    async findAccount(ctx: KoaContextWithOIDC, id: string) {
      ...
      // Priority: 1. externalAccountId 2. ctx.oidc.session?.accountId 3. the passed-in id
      ...
      // If no account ID is available, return undefined
      if (!accountIdToFind) {
    ...
    interactions: {
      ...
      url(ctx, interaction) {
        ...
        // Read the ui_locales parameter from the OIDC request (space-separated locale priorities)
        ...
        let query = '';
        if (uiLocalesRaw) {
          // Take the first preferred locale, normalize to a site-supported tag

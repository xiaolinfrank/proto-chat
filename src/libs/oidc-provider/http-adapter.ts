...
/**
 * Create Node.js HTTP request object for OIDC Provider
...
export const createNodeRequest = async (req: NextRequest): Promise<IncomingMessage> => {
  ...
  // Calculate path relative to the prefix
...
/**
 * Create Node.js HTTP response object for OIDC Provider
 * @param resolvePromise Resolve function called when the response is complete
 */
export const createNodeResponse = (resolvePromise: () => void): ResponseCollector => {
  log('Creating Node.js response collector');

  // Object for storing response state
  const state = {
...
/**
 * Create context for calling provider.interactionDetails (req, res)
...
export const createContextForInteractionDetails = async (
  ...
  // Extract hostname and protocol from baseUrl for use in headers
  ...
  // Special check for interaction session cookie
  ...
  // Note: The IP, geo, ua and other info here may be required by certain oidc-provider features.
  ...
  // 4. Create mock Node.js IncomingMessage using createNodeRequest
  // pathPrefix is set to '/' because our URL is already in the path format expected by Provider: /interaction/:uid
  ...
  // 5. Create mock Node.js ServerResponse using createNodeResponse

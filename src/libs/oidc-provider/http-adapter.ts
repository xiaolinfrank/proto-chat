...
/**
 * Create a Node.js HTTP request object for the OIDC Provider
...
export const createNodeRequest = async (req: NextRequest): Promise<IncomingMessage> => {
  ...
  // Calculate the path relative to the prefix
...
/**
 * Response collector interface for capturing the OIDC Provider's response
 */
...
/**
 * Create a Node.js HTTP response object for the OIDC Provider
 * @param resolvePromise resolver function called when the response is complete
 */
export const createNodeResponse = (resolvePromise: () => void): ResponseCollector => {
  log('Creating Node.js response collector');

  // Object to store the response state
  const state = {
...
export const createContextForInteractionDetails = async (
  ...
  // Specifically check the interaction session cookie
  ...
  // Note: The IP, geo, ua, and other information here may be required by certain oidc-provider features.
  // If issues arise, these may need to be extracted from real request headers (e.g., 'x-forwarded-for', 'user-agent')
  const interactionUrl = urlJoin(baseUrl, `/oauth/consent/${uid}`);
  log('Creating interaction URL: %s', interactionUrl);

  const mockNextRequest = {
    cookies: {
      // Simulate the NextRequestCookies interface
  ...
  // pathPrefix is set to '/' because our URL is already in the Provider's expected format /interaction/:uid
  const req: IncomingMessage = await createNodeRequest(mockNextRequest);
  // @ts-ignore - attach the parsed cookies to the mock Node.js request

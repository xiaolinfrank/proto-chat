...
/**
 * Pre-check the stdio command to capture detailed error information
 */
async function preCheckStdioCommand(params: {
  ...
  return new Promise((resolve) => {
    ...
    // Collect stderr - this is the key part
...
export class MCPClient {
  ...
  constructor(params: MCPClientParams) {
    ...
    switch (params.type) {
      case 'http': {
        ...
        // Build headers, including user-defined headers and authentication info
  ...
  async initialize(options: { onProgress?: (progress: Progress) => void } = {}) {
    ...
    try {
      ...
      // For stdio connection failures, attempt to pre-check the command for detailed error information

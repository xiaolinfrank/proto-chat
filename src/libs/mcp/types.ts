 ...
 * Supports manual configuration for the first stage and future OAuth 2.1 automated flows
 */
export interface AuthConfig {
  // C. [User Token] obtained after user authorization
  accessToken?: string;

  // Bearer Token pasted manually by the user
  // --- Stage 2 & 3: OAuth 2.1 automated flow ---
  // A. [Client Credentials] from static configuration or dynamic registration
  clientId?: string;

  clientSecret?: string;
  refreshToken?: string; // if it's a confidential client
  scope?: string; // requested permission scope, e.g., "repo user:email"

  // B. [Authorization Server Metadata] obtained via server discovery mechanism
  serverMetadata?: {

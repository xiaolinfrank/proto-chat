  ...
  const executors: Partial<Record<AgentInstruction['type'], InstructionExecutor>> = {
    ...
    call_llm: async (instruction, state) => {
      ...
      if (shouldSkipCreateMessage) {
        // Skip the first creation; subsequent calls will not skip
        ...
        // On the first regenerated creation of a userMessage, llmPayload has no parentMessageId,
        // so assign it this way
        // TODO: Perhaps this should be implemented with an init method in the future

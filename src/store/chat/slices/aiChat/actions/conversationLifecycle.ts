...
> = (set, get) => ({
  sendMessage: async ({ message, files, onlyAddUserMessage }) => {
    ...
    // Construct local media preview for server-mode temporary messages (prefer S3 URL)
    ...
    try {
      ...
      // When a new topic was created or the user cancelled the message (or it failed), there is no data
  ...
  regenerateAssistantMessage: async (id, params) => {
    ...
    // The message is from AI so we need to find its user message
    const userId = currentMessage.parentId;
    const userIndex = chats.findIndex((c) => c.id === userId);
    // If the message has no parentId, use the same mode as user

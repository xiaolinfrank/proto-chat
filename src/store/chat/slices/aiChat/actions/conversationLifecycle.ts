...
> = (set, get) => ({
  sendMessage: async ({ message, files, onlyAddUserMessage }) => {
    ...
    try {
      ...
      // Created a new topic or the user cancelled the message (or it failed), no data at this point
  ...
  regenerateAssistantMessage: async (id, params) => {
    ...
    // Message is from AI so we need to find its user message
    const userId = currentMessage.parentId;
    const userIndex = chats.findIndex((c) => c.id === userId);
    // If the message has no parentId, use the same user mode

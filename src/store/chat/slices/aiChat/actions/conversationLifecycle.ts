...
> = (set, get) => ({
  sendMessage: async ({ message, files, onlyAddUserMessage }) => {
    ...
    // Construct local media preview of server-side mode temporary messages (prefer S3 URL)
    ...
    try {
      ...
      // Created new topic or user cancelled message (or it failed), at this time there is no data
  ...
  regenerateAssistantMessage: async (id, params) => {
    ...
    // The message is sent by AI so we need to find its user message
    const userId = currentMessage.parentId;
    const userIndex = chats.findIndex((c) => c.id === userId);
    // If message doesn't have parentId, then same as user mode

...
> = (set, get) => ({
  sendMessage: async ({ message, files, onlyAddUserMessage }) => {
    ...
    // Build local media preview for temporary messages in server mode (S3 URL takes priority)
    ...
    try {
      ...
      // A new topic was created, or the user cancelled (or failed), so data is absent
  ...
  regenerateAssistantMessage: async (id, params) => {
    ...
    // The message is from the AI, so we need to find its corresponding user message

...
export interface SearchAction {
  ...
  /**
   * Re-initiate search
   * @description Will update plugin's arguments parameter, then search again
   */
  triggerSearchAgain: (
    id: string,
    data: SearchQuery,
...
> = (set, get) => ({
  ...
  saveSearchResult: async (id) => {
    ...
    const [result] = await Promise.all([
      ...
      // 2. Insert this tool call message into ai message's tools
      addToolItem(),
    ]);
    if (!result) return;
  ...
  search: async (id, params, aiSummary = true) => {
    ...
    try {
      ...
      // If aiSummary is true, will automatically trigger summary
      return aiSummary;
    } catch (error) {

...
> = (set, get) => ({
  ...
  removeGeneration: async (generationId: string) => {
    ...
    // Check if any batch became empty after deletion, and remove empty batches if so
  ...
  internal_deleteGeneration: async (generationId: string) => {
    ...
    // Find the batch that contains the target generation
      ...
      },
      {
        ...
        onSuccess: async (data: GetGenerationStatusResult) => {
          ...
          if (
            ...
            // Reset request counter since the task has completed
            ...
            // Refresh generation batches after both success and failure

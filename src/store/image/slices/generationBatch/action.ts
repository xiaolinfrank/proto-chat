...
> = (set, get) => ({
  ...
  removeGeneration: async (generationId: string) => {
    ...
    // Check if any batches became empty after deletion, and remove empty batches if so
    if (activeGenerationTopicId) {
      ...
      // If empty batches were removed, refresh data again to ensure consistency
  ...
  internal_deleteGeneration: async (generationId: string) => {
    ...
    // 2. Call backend service to delete the generation
    ...
    return useClientDataSWR<GetGenerationStatusResult>(
      ...
      },
      {
        ...
        refreshInterval: (data: GetGenerationStatusResult | undefined) => {
          ...
          // Dynamically adjust interval based on request count using exponential backoff
        ...
        onSuccess: async (data: GetGenerationStatusResult) => {
          ...
          // Find the corresponding batch; the generation DB record contains generationBatchId
          const currentBatches = get().generationBatchesMap[topicId] || [];
          const targetBatch = currentBatches.find((batch) =>
            batch.generations.some((gen) => gen.id === generationId),
          );

          // Update the corresponding generation whether status is success or error
          if (
            (data.status === AsyncTaskStatus.Success || data.status === AsyncTaskStatus.Error) &&
            targetBatch
          ) {
            // Reset request counter since the task is complete
            ...
            if (data.generation) {
              ...
              // If generation succeeded and has a thumbnail, check if the current topic has an imageUrl
              if (data.status === AsyncTaskStatus.Success && data.generation.asset?.thumbnailUrl) {
                ...
                // If the current topic has no imageUrl, update it with this generation's thumbnailUrl
            ...
            // Always refreshGenerationBatches after success or failure

...
> = (set, get) => ({
  ...
  toggleAspectRatioLock: () => {
    set(
      (state) => {
        ...
        // If transitioning from unlocked to locked and there is an active aspect ratio, immediately adjust dimensions
        if (newLockState && activeAspectRatio && parameters && parametersSchema) {
          ...
          if (
            ...
            // Adjustment is needed if current ratio doesn't match target ratio
            if (Math.abs(currentRatio - targetRatio) > 0.01) {
              ...
              if (
                ...
                // Prefer keeping width and adjusting height
                let newWidth = currentWidth;
                let newHeight = Math.round(currentWidth / targetRatio);

                // If calculated height is out of range, switch to keeping height and adjusting width
  ...
  setAspectRatio: (aspectRatio) => {
    ...
    // If the model supports width/height, calculate the new dimensions
    if (
      parametersSchema?.width &&
      parametersSchema?.height &&
  ...
  setModelAndProviderOnSelect: (model, provider) => {
    ...
    // Only remember the last selection for logged-in users, consistent with the restore strategy

...
> = (set, get) => ({
  ...
  toggleAspectRatioLock: () => {
    set(
      (state) => {
        ...
        // If transitioning from unlocked to locked and there is an active aspect ratio, adjust size immediately
        if (newLockState && activeAspectRatio && parameters && parametersSchema) {
          ...
          if (
            ...
            // Adjustment needed if current ratio does not match target ratio
            if (Math.abs(currentRatio - targetRatio) > 0.01) {
              ...
              if (
                ...
                // If calculated height is out of range, keep height instead and adjust width
  ...
  setModelAndProviderOnSelect: (model, provider) => {
    ...
    // Only remember last selection for logged-in users, consistent with the restore strategy

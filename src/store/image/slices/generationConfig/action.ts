...
> = (set, get) => ({
          ...
          if (
            ...
            if (Math.abs(currentRatio - targetRatio) > 0.01) {
              ...
              if (
                ...
                // Prioritize maintaining width, adjust height
                let newWidth = currentWidth;
                let newHeight = Math.round(currentWidth / targetRatio);

                // If calculated height exceeds range, maintain height and adjust width instead
  ...
  setModelAndProviderOnSelect: (model, provider) => {
    ...
    // Remember last selection only for logged-in users, consistent with restore strategy
  ...
  _initializeDefaultImageConfig: () => {
    ...
    // Try to select the first available image provider/model as default

...
> = (set, get) => ({
  ...
  toggleAspectRatioLock: () => {
    set(
      (state) => {
        ...
        // If switching from unlocked to locked and there is an active aspect ratio, adjust dimensions immediately
        if (newLockState && activeAspectRatio && parameters && parametersSchema) {
          ...
          // Only adjust when both width and height are present
          if (
            ...
            // Adjust if the current ratio does not match the target ratio
            if (Math.abs(currentRatio - targetRatio) > 0.01) {
              ...
              if (
                ...
                // Prefer keeping the width and adjusting the height
                let newWidth = currentWidth;
                let newHeight = Math.round(currentWidth / targetRatio);

                // If the calculated height is out of range, keep the height and adjust the width instead
  ...
  setAspectRatio: (aspectRatio) => {
    ...
    // If the model supports width/height, calculate new dimensions
    if (
      parametersSchema?.width &&
      parametersSchema?.height &&
    ...
    // If the model itself supports aspectRatio, update it
  ...
  setModelAndProviderOnSelect: (model, provider) => {
    ...
    // Only remember the last selection for logged-in users, consistent with the restoration strategy
  ...
  _initializeDefaultImageConfig: () => {
    ...
    // Try to select the first available image provider/model as the default

...
/**
 * Calculate the most suitable dimensions based on target ratio and default size
...
export function adaptSizeToRatio(ratio: number, defaultWidth: number, defaultHeight: number) {
  ...
  if (ratio > currentRatio) {
    // Target ratio is wider, maintain width and adjust height
    return { width: defaultWidth, height: Math.round(defaultWidth / ratio) };
  } else {
    // Target ratio is taller, maintain height and adjust width

 ...
 * @param ratio - A ratio string in the format "width:height"
 * @returns The numeric ratio value, or 1:1 ratio on error
 */
...
/**
 * Calculate the most suitable dimensions based on the target ratio and default size
...
export function adaptSizeToRatio(ratio: number, defaultWidth: number, defaultHeight: number) {
  ...
  if (ratio > currentRatio) {
    // Target ratio is wider: keep width, adjust height
    return { width: defaultWidth, height: Math.round(defaultWidth / ratio) };
  } else {
    // Target ratio is taller: keep height, adjust width

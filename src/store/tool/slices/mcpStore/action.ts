...
> = (set, get) => ({
  cancelInstallMCPPlugin: async (identifier) => {
    // Get and abort the AbortController
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        // Resume mode: retrieve previous information from storage
        ...
        // 🌐 Check if cloudEndPoint exists: web + stdio type + haveCloudEndpoint present
        ...
        if (hasCloudEndpoint) {
          ...
          // ❌ stdio type: requires full system dependency check process
          ...
          if (!skipDepsCheck && !result.allDependenciesMet) {
            // Dependencies not met, pause installation process and show dependency install guide
            updateMCPInstallProgress(identifier, {
            ...
            // Pause installation process, waiting for user to install dependencies
            updateInstallLoadingState(identifier, undefined);
          ...
          if (result.needsConfig) {
            ...
            // Pause installation process, waiting for user configuration
            updateInstallLoadingState(identifier, undefined);
      ...
      // set version
      if (manifest) {
        // set Version - Use semver to compare version numbers and use the larger one
        ...
        if (dataVersion && manifestVersion) {
          ...
          if (valid(dataVersion) && valid(manifestVersion)) {
            ...
            // If the version format is invalid, prefer dataVersion
            manifest.version = dataVersion;
          }
        } else {
          // If only one version exists, use the existing one
      ...
      // Update connection object, write the merged configuration
      ...
      // If it is a structured MCPError
      if (!!error.data && 'errorData' in error.data) {
        ...
        // Fallback handling for generic errors
      ...
      // Set error state, show structured error information
      updateMCPInstallProgress(identifier, {
        errorInfo,
        progress: 0,

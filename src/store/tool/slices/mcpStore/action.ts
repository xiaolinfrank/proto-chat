  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        ...
        // 🌐 Check for cloudEndPoint: web client + stdio type + haveCloudEndpoint present
        ...
        if (hasCloudEndpoint) {
          ...
          // ❌ stdio type: requires full system dependency check
      ...
      // set version
      if (manifest) {
        // set Version - use semver to compare and pick the larger version
      ...
      // Update connection object with merged config
      ...
      await pluginService.installPlugin({
        // For MCP, store connection info in customParams first
      ...
      // Set error state, show structured error info
      updateMCPInstallProgress(identifier, {
        errorInfo,
        progress: 0,

...
const buildCloudMcpManifest = (params: {
  ...
  // If tools are in MCP format, convert them to LobeChat api format
...
> = (set, get) => ({
  cancelInstallMCPPlugin: async (identifier) => {
    ...
    if (abortController) {
      ...
      // Clear the AbortController from storage
      set(
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    // Create an AbortController for cancelling the installation
    ...
    try {
      ...
      if (resume) {
        // Resume mode: retrieve previously stored info
        ...
        if (hasCloudEndpoint) {
          // 🌐 Use cloudEndPoint, create a cloud type connection
          ...
          // Step 3: Check whether system dependencies are met
          if (!skipDepsCheck && !result.allDependenciesMet) {
            // Dependencies not met; pause installation and show dependency install guide
            updateMCPInstallProgress(identifier, {
            ...
            // Pause installation flow, waiting for user to install dependencies
            updateInstallLoadingState(identifier, undefined);
            return false; // Return false to indicate dependencies need to be installed
          }

          // Step 4: Check whether configuration is required
          if (result.needsConfig) {
            // Configuration required; pause installation flow
            updateMCPInstallProgress(identifier, {
              checkResult: result,
            ...
            // Pause installation flow, waiting for user configuration
            updateInstallLoadingState(identifier, undefined);
      ...
      // set version
      if (manifest) {
        // set Version - use semver to compare and pick the greater version
        const dataVersion = data?.version;
        const manifestVersion = manifest.version;

        if (dataVersion && manifestVersion) {
          // Both versions exist; compare and use the greater one
          ...
          // Only one version exists; use whichever is present
      ...
      // Update the connection object with the merged config
      ...
      await pluginService.installPlugin({
        // For MCP, store the connection info in the customParams field first
      ...
      // Briefly show completed state before clearing progress
      await sleep(1000);
      ...
      // If this is a structured MCPError
      ...
      // Set error state and display structured error info
      updateMCPInstallProgress(identifier, {
        errorInfo,
        progress: 0,
  ...
  testMcpConnection: async (params) => {
    ...
    // Create an AbortController for cancelling the test
    const abortController = new AbortController();

    // Store the AbortController and set loading state
    set(
      produce((draft: MCPStoreState) => {

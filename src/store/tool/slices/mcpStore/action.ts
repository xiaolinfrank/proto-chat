...
> = (set, get) => ({
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        ...
        if (hasCloudEndpoint) {
          ...
          if (!skipDepsCheck && !result.allDependenciesMet) {
            // Dependencies not met, pause install flow and show dependency installation guide
            updateMCPInstallProgress(identifier, {
            ...
            updateInstallLoadingState(identifier, undefined);
            return false; // returns false indicating dependencies need to be installed
          }
          ...
          if (result.needsConfig) {
            ...
            updateInstallLoadingState(identifier, undefined);
            return false; // returns false indicating configuration is needed
          }
        ...
        manifest = await mcpService.getStdioMcpServerManifest(
          {
            ...
            env: mergedStdioEnv,
            name: identifier, // pass config as env vars (resume mode)
          },
      ...
      // set version
      if (manifest) {
        // set Version - use semver to compare versions and take the larger one
        ...
        if (dataVersion && manifestVersion) {
          ...
          // If only one version exists, use the available one
      ...
      if (!!error.data && 'errorData' in error.data) {
        ...
        // Fallback handling for plain errors

...
const buildCloudMcpManifest = (params: {
  ...
  // If using MCP format tools, convert to LobeChat api format
...
> = (set, get) => ({
  cancelInstallMCPPlugin: async (identifier) => {
    ...
    // Clear installation progress and loading state
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        ...
        // Check if cloudEndPoint exists: web client + stdio type + haveCloudEndpoint
        ...
        if (hasCloudEndpoint) {
          ...
          if (!skipDepsCheck && !result.allDependenciesMet) {
            ...
            updateInstallLoadingState(identifier, undefined);
            return false; // Return false to indicate dependencies need installation
          }

          // Step 4: Check if configuration is required
          if (result.needsConfig) {
            // Configuration required, pause installation
            updateMCPInstallProgress(identifier, {
              checkResult: result,
            ...
            updateInstallLoadingState(identifier, undefined);
            return false; // Return false to indicate configuration required
          }
        ...
        manifest = await mcpService.getStdioMcpServerManifest(
          {
            ...
            env: mergedStdioEnv,
            name: identifier, // Pass configuration as environment variable (in resume mode)
          },
      ...
      // Briefly display completion status before clearing progress
      await sleep(1000);
      ...
      // If error due to cancellation, handle silently
      if (abortController.signal.aborted) {
      ...
      if (!!error.data && 'errorData' in error.data) {
        ...
        // Fallback to handle regular errors

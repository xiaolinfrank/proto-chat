...
> = (set, get) => ({
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        // Resume mode: get previous info from store
        ...
        // 🌐 Check for cloudEndPoint: web client + stdio type + haveCloudEndpoint exists
        ...
        if (hasCloudEndpoint) {
          ...
          if (!skipDepsCheck && !result.allDependenciesMet) {
            // Dependencies not met, pause install process and show dependency install guide
            updateMCPInstallProgress(identifier, {
            ...
            // Pause install process, wait for user to install dependencies
            updateInstallLoadingState(identifier, undefined);
            return false; // Return false indicating dependencies need to be installed
          }

          // Step 4: Check if configuration is required
          if (result.needsConfig) {
            // Configuration required, pause install process
            updateMCPInstallProgress(identifier, {
              checkResult: result,
            ...
            // Pause install process, wait for user configuration
            updateInstallLoadingState(identifier, undefined);
            return false; // Return false indicating configuration is required
          }
      ...
      // Update connection object, write merged config
      ...
      // Set error state, display structured error info
      updateMCPInstallProgress(identifier, {
        errorInfo,
        progress: 0,

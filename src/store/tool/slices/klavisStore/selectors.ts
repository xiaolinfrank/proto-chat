...
export const klavisStoreSelectors = {
  /**
   * Get the set of identifiers for all Klavis servers
   */
  ...
  /**
   * Check whether a given identifier belongs to a Klavis server
   * @param identifier - Server identifier (e.g., 'google-calendar')
   */
  isKlavisServer:
    (identifier: string) =>
  ...
  /**
   * Check whether a server is loading
   * @param identifier - Server identifier (e.g., 'google-calendar')
   */
  ...
  /**
   * Check whether a tool is currently executing
   */
  ...
  klavisAsLobeTools: (s: ToolStore) => {
    ...
    servers.forEach((server) => {
      ...
      if (apis.length > 0) {
        tools.push({
          // Use identifier as the storage and reference key

...
> = (set, get) => ({
  callKlavisTool: async (params) => {
    ...
    try {
      // Call tRPC server endpoint to execute the tool (using toolsClient for longer timeout)
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      // Call tRPC server endpoint to create a single server instance
      ...
      set(
        produce((draft: KlavisStoreState) => {
          // Check if it already exists (by identifier), update if so

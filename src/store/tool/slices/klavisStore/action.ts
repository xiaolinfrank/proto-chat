...
> = (set, get) => ({
  callKlavisTool: async (params) => {
    ...
    try {
      // Call tRPC server-side interface to execute tool (using toolsClient for longer timeout)
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      // Call tRPC server-side interface to create a single server instance
    ...
    try {
      ...
      if (!instanceStatus.isAuthenticated) {
        if (!instanceStatus.authNeeded) {
          // If authentication is not required, everything is fine
          return;
        }

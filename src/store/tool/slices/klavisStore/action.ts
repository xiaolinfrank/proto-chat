...
export interface KlavisStoreAction {
  ...
  /**
   * Refresh tool list for Klavis Server
   * @param identifier - Server identifier (e.g., 'google-calendar')
   */
...
> = (set, get) => ({
  callKlavisTool: async (params) => {
    ...
    try {
      // Call tRPC server endpoint to execute tool (using toolsClient for longer timeout)
  ...
  refreshKlavisServerTools: async (identifier) => {
    ...
    try {
      // First check server's authentication status

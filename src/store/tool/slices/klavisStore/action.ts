...
export interface KlavisStoreAction {
  ...
  /**
   * Create an individual Klavis MCP Server instance
   * @returns Created server instance, or an object with oauthUrl if OAuth is required
   */
...
> = (set, get) => ({
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      // Call tRPC server interface to create an individual server instance
      ...
      set(
        produce((draft: KlavisStoreState) => {
          // Check if already exists (using identifier), if so update
  ...
  refreshKlavisServerTools: async (identifier) => {
    ...
    try {
      // First check the server authentication status
      const instanceStatus = await lambdaClient.klavis.getServerInstance.query({
        instanceId: server.instanceId,
      });

      // If authentication fails, remove server and reset state
      if (!instanceStatus.isAuthenticated) {
        if (!instanceStatus.authNeeded) {
          // If no authentication is needed, everything is fine
          return;
        }
      ...
      // Authentication successful, fetch tool list (using toolsClient for longer timeout)

...
export interface KlavisStoreAction {
   ...
   * @returns The created server instance, or an object with oauthUrl if OAuth is needed
   */
...
> = (set, get) => ({
  ...
  completeKlavisServerAuth: async (identifier) => {
    // Refresh tool list after OAuth completion
    ...
    try {
      ...
      if (!instanceStatus.isAuthenticated) {
        if (!instanceStatus.authNeeded) {
          // If authentication not needed, it's fine
          return;
        }

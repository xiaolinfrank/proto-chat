...
> = (set, get) => ({
  callKlavisTool: async (params) => {
    ...
    try {
      // Call the tRPC server-side interface to execute the tool (using toolsClient for a longer timeout)
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      ...
      set(
        produce((draft: KlavisStoreState) => {
          // Check if already exists (using identifier); if so, update it
    ...
    try {
      ...
      if (!instanceStatus.isAuthenticated) {
        if (!instanceStatus.authNeeded) {
          // If no auth is needed, everything is fine
          return;
        }

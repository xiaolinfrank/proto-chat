...
export interface KlavisStoreAction {
  ...
  /**
   * Refresh the tool list of a Klavis Server
   * @param identifier - Server identifier (e.g., 'google-calendar')
   */
  ...
  /**
   * Use SWR to fetch the user's Klavis server list
...
> = (set, get) => ({
  callKlavisTool: async (params) => {
    ...
    try {
      // Call the tRPC server interface to execute the tool (using toolsClient for a longer timeout)
  ...
  completeKlavisServerAuth: async (identifier) => {
    // After OAuth is complete, refresh the tool list
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      // Call the tRPC server interface to create a single server instance
      ...
      set(
        produce((draft: KlavisStoreState) => {
          // Check if already exists (using identifier), update if so
  ...
  refreshKlavisServerTools: async (identifier) => {
    ...
    try {
      ...
      // If authentication failed, delete the server and reset state
      if (!instanceStatus.isAuthenticated) {
        if (!instanceStatus.authNeeded) {
          // If authentication is not needed, no issue
          return;
        }
      ...
      // Authentication succeeded, fetch tool list (using toolsClient for a longer timeout)

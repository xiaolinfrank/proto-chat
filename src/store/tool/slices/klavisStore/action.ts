...
> = (set, get) => ({
  callKlavisTool: async (params) => {
    ...
    try {
      // Call the tRPC server-side interface to execute the tool (using toolsClient for longer timeout)
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      // Call the tRPC server-side interface to create a single server instance
  ...
  refreshKlavisServerTools: async (identifier) => {
    ...
    try {
      ...
      // Authentication succeeded, fetch tool list (using toolsClient for longer timeout)
      ...
      // Update tools list and authentication status in the database
      ...
      },
      {
        ...
        onSuccess: (data) => {
          if (data.length > 0) {
            set(
              produce((draft: KlavisStoreState) => {
                // Check for existing entries by identifier

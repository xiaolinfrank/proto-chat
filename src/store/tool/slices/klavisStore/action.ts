...
export interface KlavisStoreAction {
  ...
  /**
   * Update server status after OAuth authentication is complete
   ...
   * @returns The created server instance, or an object with oauthUrl if OAuth is required
   */
  ...
  /**
   * Delete a Klavis Server
   * @param identifier - Server identifier (e.g., 'google-calendar')
   */
  removeKlavisServer: (identifier: string) => Promise<void>;

  /**
   * Fetch the user's Klavis server list using SWR
...
> = (set, get) => ({
  callKlavisTool: async (params) => {
    ...
    try {
      // Call the tRPC server endpoint to execute the tool (use toolsClient for longer timeout)
  ...
  completeKlavisServerAuth: async (identifier) => {
    // After OAuth completes, refresh the tool list
  ...
  createKlavisServer: async (params) => {
    ...
    try {
      // Call the tRPC server endpoint to create a single server instance
      ...
      set(
        produce((draft: KlavisStoreState) => {
          // Check if already exists (by identifier); update if so
  ...
  refreshKlavisServerTools: async (identifier) => {
    ...
    try {
      ...
      // If authentication failed, remove the server and reset state
      if (!instanceStatus.isAuthenticated) {
        if (!instanceStatus.authNeeded) {
          // Auth is not required, so there is no problem
          return;
        }

        // Remove from local state (by identifier)
        set(
          produce((draft: KlavisStoreState) => {
      ...
      // Authentication succeeded; fetch the tool list (use toolsClient for longer timeout)
      ...
      // Update the tool list and authentication status in the database

...
`);
    ...
    const rawKey = Buffer.from(KEY_VAULTS_SECRET, 'base64'); // Ensure the key is 32 bytes (256 bits)
  ...
  encrypt = async (keyVault: string): Promise<string> => {
    const iv = crypto.getRandomValues(new Uint8Array(12)); // For GCM, a 12-byte IV is recommended
    ...
    const authTag = buffer.slice(-16); // The auth tag is the last 16 bytes of the encrypted data
    const encrypted = buffer.slice(0, -16); // The rest is the encrypted data

    return `${Buffer.from(iv).toString('hex')}:${authTag.toString('hex')}:${encrypted.toString('hex')}`;
  };

  // Assumes the key and encrypted data are obtained externally

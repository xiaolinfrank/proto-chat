...
export const userRouter = router({
  ...
  getUserState: userProcedure.query(async ({ ctx }): Promise<UserInitializationState> => {
    ...
    return {
      ...
      // If there are messages or an assistant has been created, consider it a conversation
  ...
  updateAvatar: userProcedure.input(z.string()).mutation(async ({ ctx, input }) => {
    // If it's Base64 data, upload to S3
    if (input.startsWith('data:image')) {
      try {
        ...
        // Get the old avatar URL to delete it afterwards
        ...
        // Convert Base64 data to a Buffer before uploading to S3
    ...
    // If it's not Base64 data, update the user avatar directly with the URL

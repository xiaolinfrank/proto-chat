...
export const userRouter = router({
  ...
  getUserState: userProcedure.query(async ({ ctx }): Promise<UserInitializationState> => {
    ...
    return {
      ...
      // If there are messages or an assistant has been created, considered to have a conversation
  ...
  updateAvatar: userProcedure.input(z.string()).mutation(async ({ ctx, input }) => {
    // If it's Base64 data, it needs to be uploaded to S3
    if (input.startsWith('data:image')) {
      try {
        ...
        // Use UUID to generate unique filename to prevent cache issues
        // Get old avatar URL to delete it later
    ...
    // If not Base64 data, directly update user avatar with the URL

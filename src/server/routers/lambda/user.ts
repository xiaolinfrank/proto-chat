...
export const userRouter = router({
  ...
  getUserState: userProcedure.query(async ({ ctx }): Promise<UserInitializationState> => {
    ...
    return {
      ...
      // If there are messages or an assistant has been created, consider it as having a conversation
  ...
  updateAvatar: userProcedure.input(z.string()).mutation(async ({ ctx, input }) => {
    ...
    if (input.startsWith('data:image')) {
      try {
        ...
        // Split string and get the Base64 part
    ...
    // If it's not Base64 data, directly use the URL to update user avatar

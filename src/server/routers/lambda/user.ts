...
export const userRouter = router({
  ...
  updateAvatar: userProcedure.input(z.string()).mutation(async ({ ctx, input }) => {
    // If it is Base64 data, it needs to be uploaded to S3
    if (input.startsWith('data:image')) {
      try {
        ...
        // Generate a unique filename using UUID to prevent caching issues
    ...
    // If it is not Base64 data, update the user avatar directly with the URL

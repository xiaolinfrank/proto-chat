...
export const userRouter = router({
  ...
  getUserState: userProcedure.query(async ({ ctx }): Promise<UserInitializationState> => {
    ...
    return {
      ...
      // Considered to have a conversation if there are messages or an assistant has been created
  ...
  updateAvatar: userProcedure.input(z.string()).mutation(async ({ ctx, input }) => {
    // If it's Base64 data, upload it to S3
    if (input.startsWith('data:image')) {
      try {
        // Extract the mimeType, e.g. "image/png"
        const prefix = 'data:';
        ...
        // Retrieve old avatar URL so it can be deleted later
    ...
    // If not Base64 data, update the user avatar directly with the URL

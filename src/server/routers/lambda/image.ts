...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // Normalize reference image addresses, uniformly store S3 keys (avoid storing expiring pre-signed URLs in database)
    ...
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      const newBatch: NewGenerationBatch = {
        ...
        provider,
        userId,
        width: params.width, // Use converted configuration to store in database
      };
      ...
      // 2. Create 4 generations (first phase fixed at 4 images)
      const seeds =
        'seed' in params
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          // Create asyncTask directly within transaction

...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // Normalize reference image URLs, storing S3 keys uniformly (to avoid storing expiring pre-signed URLs in the database)
    let configForDatabase = { ...params };
    // 1) Process multi-image imageUrls
    ...
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      const newBatch: NewGenerationBatch = {
        ...
        provider,
        userId,
        width: params.width, // Store to database using the converted config
      };
      ...
      // 2. Create N generations (fixed count per batch)
      const seeds =
        'seed' in params
      ...
      // 3. Concurrently create an asyncTask for each generation (within the transaction)
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          // Directly create the asyncTask within the transaction
    ...
    try {
      ...
      // If the overall process fails, update all task statuses to failed
      try {

...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // Normalize reference image URLs, store as S3 keys (avoid storing expiring pre-signed URLs in database)
    let configForDatabase = { ...params };
    // 1) Process multiple-image imageUrls
    ...
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      // 2. Create generations (fixed count per batch)
      const seeds =
        'seed' in params
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          // Directly create asyncTask within transaction
    ...
    try {
      ...
      // If overall fails, update all task statuses to failed
      try {

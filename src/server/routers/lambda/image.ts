...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // 1) Process multiple image URLs (imageUrls)
    ...
    // 2) Process single image URL (imageUrl)
    ...
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      // 2. Create N generations (fixed number of images generated per batch)
      const seeds =
        'seed' in params
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          ...
          // Update the asyncTaskId for the generation
          await tx
            .update(generations)
            .set({ asyncTaskId })

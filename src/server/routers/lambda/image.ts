...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // Normalize reference image addresses, uniformly store S3 keys (to avoid storing expiring pre-signed URLs in the database)
    let configForDatabase = { ...params };
    // 1) Handle multiple image imageUrls
    ...
    // 2) Handle single image imageUrl
    ...
    // Defensive check: ensure no full URL enters the database
    validateNoUrlsInConfig(configForDatabase, 'configForDatabase');

    // Step 1: Atomically create all database records within a transaction
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      const newBatch: NewGenerationBatch = {
        ...
        provider,
        userId,
        width: params.width, // Store to database using the converted configuration
      };
      ...
      // 2. Create 4 generations (phase one always generates 4 images)
      const seeds =
        'seed' in params
      ...
      // 3. Concurrently create asyncTask for each generation (within the transaction)
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          // Create asyncTask directly within the transaction
          ...
          // Update the asyncTaskId of the generation
          await tx
            .update(generations)
            .set({ asyncTaskId })
    ...
    try {
      ...
      // Create a caller using the unified caller factory
      ...
      // If the overall operation fails, update all task statuses to failed
      try {

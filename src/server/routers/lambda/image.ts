...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // Normalize reference image addresses, uniformly store S3 key (avoid storing pre-signed URLs that will expire into the database)
    let configForDatabase = { ...params };
    // 1) Process multiple image URLs
    ...
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      const newBatch: NewGenerationBatch = {
        ...
        provider,
        userId,
        width: params.width, // Store to database using converted configuration
      };
      ...
      // 2. Create 4 generations (phase 1 generates a fixed 4 images)
      const seeds =
        'seed' in params
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          // Directly create asyncTask within the transaction
          ...
          // Update the asyncTaskId of generation
          await tx
            .update(generations)
            .set({ asyncTaskId })

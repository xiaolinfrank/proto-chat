...
export const imageRouter = router({
  createImage: imageProcedure.input(createImageInputSchema).mutation(async ({ input, ctx }) => {
    ...
    // Normalize reference image URLs, storing S3 keys uniformly (to avoid saving expiring pre-signed URLs in the database)
    let configForDatabase = { ...params };
    // 1) Handle multiple imageUrls
    ...
    // Defensive check: ensure no full URLs are stored in the database
    ...
    const { batch: createdBatch, generationsWithTasks } = await serverDB.transaction(async (tx) => {
      ...
      const newBatch: NewGenerationBatch = {
        ...
        provider,
        userId,
        width: params.width, // Store the converted config to the database
      };
      ...
      // 2. Create generations (phase 1 always generates a fixed number of images)
      const seeds =
        'seed' in params
      ...
      const generationsWithTasks = await Promise.all(
        createdGenerations.map(async (generation) => {
          ...
          // Update the asyncTaskId on the generation record
          await tx
            .update(generations)
            .set({ asyncTaskId })
    ...
    try {
      ...
      // Use the unified caller factory to create a caller
      ...
      // If the whole process fails, update all task statuses to error
      try {

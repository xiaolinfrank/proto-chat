...
export const klavisRouter = router({
  ...
  createServerInstance: klavisProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const isAuthenticated = !oauthUrl; // If there is no oauthUrl, authentication is not required or already authenticated
  ...
  deleteServerInstance: klavisProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Call Klavis API to delete the server instance
  ...
  updateKlavisPlugin: klavisProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Get the existing plugin (using identifier)
      const existingPlugin = await ctx.pluginModel.findById(identifier);

      // Build the manifest containing all tools
      const manifest: LobeChatPluginManifest = {
        api: tools.map((tool) => ({
          description: tool.description || '',
          name: tool.name,

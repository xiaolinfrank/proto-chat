  ...
  registerM2MToken: marketProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        // Set client-readable status marker cookie (does not contain actual token)
        ...
        // Set Set-Cookie headers via context's resHeaders
  ...
  reportCall: marketProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        // Don't throw error, reporting failure should not affect main flow
        return { success: false };
      }
    }),

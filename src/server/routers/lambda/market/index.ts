  ...
  registerM2MToken: marketProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        // Set a client-readable status flag cookie (does not include the actual token)
        ...
        // Set the Set-Cookie header via the context's resHeaders
  ...
  reportCall: marketProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        // Do not throw error, as reporting failure should not affect the main flow
        return { success: false };
      }
    }),

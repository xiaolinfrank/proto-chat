...
/**
 * FLUX dual-CLIP prompt intelligent splitting tool
 * Splits a single prompt into separate inputs for T5-XXL and CLIP-L
 */
export function splitPromptForDualCLIP(prompt: string): {
  ...
  for (const compound of compoundStyles) {
    ...
    if (index !== -1) {
      // Found compound style, extract the corresponding original phrase
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    for (const keyword of styleKeywords) {
      ...
      if (keywordWords.length === 1) {
        // Single-word match (including normalized terms)
  ...
  if (stylePhrases.length > 0) {
    // Deduplicate at phrase level, preserving the integrity of multi-word phrases
    const uniquePhrases = [...new Set(stylePhrases)];
    return {
      // CLIP-L focuses on style and visual concepts
      clipLPrompt: uniquePhrases.join(' '),
      // T5-XXL receives the full context to understand semantic relationships
      t5xxlPrompt: prompt,
    };
  }

  // Fallback when no style words are found: use the same prompt (ensures compatibility)
  return {
    clipLPrompt: prompt,
    t5xxlPrompt: prompt,
  };

...
/**
 * FLUX dual-CLIP prompt smart splitting tool
...
export function splitPromptForDualCLIP(prompt: string): {
  ...
  if (stylePhrases.length > 0) {
    // Phrase-level deduplication, preserving multi-word phrase integrity
    const uniquePhrases = [...new Set(stylePhrases)];
    return {
      // CLIP-L focuses on styles and visual concepts

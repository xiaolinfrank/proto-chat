...
export class DiscoverService {
  ...
  async fetchM2MToken(params: { clientId: string; clientSecret: string }) {
    // Create a new MarketSDK instance using the passed client credentials
  ...
  /**
   * Select the model with the most complete abilities from a model array
  ...
  private selectModelWithBestAbilities = (models: DiscoverModelItem[]): DiscoverModelItem => {
    ...
    models.forEach((model) => {
      ...
      if (score > maxAbilitiesScore) {
        ...
        // Merge abilities with same score to ensure the most complete combination
    ...
    // Find the maximum contextWindowTokens
  ...
  private _getModelList = async (category?: string): Promise<DiscoverModelItem[]> => {
    ...
    // Optimize deduplication logic: select model with most complete abilities
    ...
    // 2. Select model with most complete abilities from each identifier group
    ...
    // 4. Select model with most complete abilities from each displayName group

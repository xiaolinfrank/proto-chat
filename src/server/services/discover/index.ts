...
export class DiscoverService {
  ...
  async fetchM2MToken(params: { clientId: string; clientSecret: string }) {
    // Create a new MarketSDK instance with the passed client credentials
  ...
  /**
   * Calculate completeness score for ModelAbilities
  ...
  /**
   * Select the model with the most complete abilities from an array of models
   * Combines the most complete abilities with the largest contextWindowTokens
   */
  private selectModelWithBestAbilities = (models: DiscoverModelItem[]): DiscoverModelItem => {
    ...
    // Assemble the final model with the best attributes
  ...
  private _getModelList = async (category?: string): Promise<DiscoverModelItem[]> => {
    ...
    // Optimize deduplication logic: select models with the most complete abilities
    ...
    // 2. Select the one with the most complete abilities from each identifier group
    ...
    // 4. Select the one with the most complete abilities from each displayName group

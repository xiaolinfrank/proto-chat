...
export class DiscoverService {
   ...
   * A higher score indicates more complete abilities
   */
  ...
  /**
   * Select the model with the most complete abilities from the model array
   * Combines the most complete abilities and the largest contextWindowTokens
   */
  private selectModelWithBestAbilities = (models: DiscoverModelItem[]): DiscoverModelItem => {
    ...
    // Select a base model (typically the first one)
    const baseModel = models[0];

    // Assemble the final model using the best attributes from each group
  ...
  private _getRawModelList = async (): Promise<DiscoverModelItem[]> => {
    ...
    const result = LOBE_DEFAULT_MODEL_LIST.map((item) => {
      ...
      // Use simple merging instead of DEFAULT_DISCOVER_MODEL_ITEM to avoid type conflicts
      return {
        ...model,
  ...
  private _getModelList = async (category?: string): Promise<DiscoverModelItem[]> => {
    ...
    // Optimize deduplication logic: select the model with the most complete abilities
    ...
    // 2. From each identifier group, select the one with the most complete abilities
    ...
    // 4. From each displayName group, select the one with the most complete abilities

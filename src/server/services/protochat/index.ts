 ...
 * ProtoChat Service
 *
 * Provides model mapping, pricing queries, usage logging, and related functionality
 */
export class ProtoChatService {
  ...
  async getModelMapping(modelId: string): Promise<ProtoChatModelMapping> {
    ...
    if (!model.length && !modelId.startsWith('protochat::')) {
      // Try matching full IDs that end with the given model ID (e.g. 'protochat::a1::gemini-2.5-flash' matches 'gemini-2.5-flash')
      model = await this.db
        .select()
        .from(protochatModels)
        .where(
  ...
  /**
   * Get a list of all enabled ProtoChat models
  ...
  /**
   * Check if the provider is a ProtoChat provider
   */

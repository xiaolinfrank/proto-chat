 ...
 * ProtoChat service
 *
 * Provides model mapping, pricing queries, usage logging, and more
 */
export class ProtoChatService {
  ...
  async getModelMapping(modelId: string): Promise<ProtoChatModelMapping> {
    ...
    if (!model.length && !modelId.startsWith('protochat::')) {
      // Try to match the full ID ending with this model ID (e.g. 'protochat::a1::gemini-2.5-flash' matches 'gemini-2.5-flash')
      model = await this.db
        .select()
        .from(protochatModels)
        .where(
   ...
   * @returns Total cost (credits), returns 0 if free model
   */
  async calculateCost(
    modelId: string,
    inputTokens: number,
